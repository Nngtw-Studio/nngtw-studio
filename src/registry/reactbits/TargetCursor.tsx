...
export interface TargetCursorProps {
  ...
  /** Per-target color decision (e.g. contrast-aware). Takes priority over cursorColorOnTarget when provided. */
  getTargetColor?: (target: Element) => string | undefined;
  /** Where the cursor first appears, before any real mousemove has occurred. Defaults to viewport center. */
...
const TargetCursor = ({
  ...
  useEffect(() => {
    ...
    const startX = initialPosition?.x ?? window.innerWidth / 2;
    const startY = initialPosition?.y ?? window.innerHeight / 2;
    gsap.set(cursor, {
      xPercent: -50,
      yPercent: -50,
      x: startX - initialOffset.x,
      y: startY - initialOffset.y,
    });
    ...
    const tickerFn = () => {
      ...
      // Re-measure every frame (rather than once on enter) so the brackets
      // keep tracking targets that move under CSS transform after lock-on —
      // e.g. a magnetic-hover button that keeps shifting toward the cursor.
    ...
    const enterHandler = (e: MouseEvent) => {
      ...
      const resolvedTargetColor = getTargetColor ? getTargetColor(target) : cursorColorOnTarget;
      ...
      const leaveHandler = () => {
        ...
        if ((cursorColorOnTarget || getTargetColor) && cornersRef.current) {
